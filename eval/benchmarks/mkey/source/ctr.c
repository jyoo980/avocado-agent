...
__CPROVER_requires(__CPROVER_is_fresh(iv, 16 * sizeof(u8)))
__CPROVER_assigns(__CPROVER_object_upto(ctx->iv, 16))
__CPROVER_ensures(__CPROVER_forall { int k; (0 <= k && k < 16) ==> ctx->iv[k] == iv[k] })
{
    memcpy(ctx->iv, iv, 16);
}
...
void ctr_add_counter( ctr_aes_context* ctx,
                      u32 carry )
__CPROVER_requires(__CPROVER_is_fresh(ctx, sizeof(*ctx)))
__CPROVER_assigns(__CPROVER_object_upto(ctx->ctr, 16))
{
    u32 counter[4];
    u32 sum;
    int i;
...
void ctr_set_counter( ctr_aes_context* ctx,
                      u8 ctr[16] )
__CPROVER_requires(__CPROVER_is_fresh(ctx, sizeof(*ctx)))
__CPROVER_requires(__CPROVER_is_fresh(ctr, 16 * sizeof(u8)))
__CPROVER_assigns(__CPROVER_object_upto(ctx->ctr, 16))
__CPROVER_ensures(__CPROVER_forall { int k; (0 <= k && k < 16) ==> ctx->ctr[k] == ctr[k] })
{
...
void ctr_init_counter( ctr_aes_context* ctx,
                       ...
                       u8 ctr[16] )
__CPROVER_requires(__CPROVER_is_fresh(ctx, sizeof(*ctx)))
__CPROVER_requires(__CPROVER_is_fresh(key, 16 * sizeof(u8)))
__CPROVER_requires(__CPROVER_is_fresh(ctr, 16 * sizeof(u8)))
__CPROVER_assigns(__CPROVER_object_upto(ctx->ctr, 16), ctx->aes)
{
...
void ctr_crypt_counter_block( ctr_aes_context* ctx,
                              ...
                              u8 output[16] )
__CPROVER_requires(__CPROVER_is_fresh(ctx, sizeof(*ctx)))
__CPROVER_requires(__CPROVER_is_fresh(output, 16 * sizeof(u8)))
__CPROVER_requires(__CPROVER_is_fresh(input, 16 * sizeof(u8)))
__CPROVER_assigns(__CPROVER_object_upto(ctx->ctr, 16),
                  __CPROVER_object_upto(output, 16),
                  ctx->aes)
{
    int i;
    u8 stream[16];
...
void ctr_crypt_counter( ctr_aes_context* ctx,
                        ...
                        u8* output,
                        u32 size )
__CPROVER_requires(size <= 32)
__CPROVER_requires(__CPROVER_is_fresh(ctx, sizeof(*ctx)))
__CPROVER_requires(__CPROVER_is_fresh(input, (size_t)size))
__CPROVER_requires(__CPROVER_is_fresh(output, (size_t)size))
__CPROVER_assigns(__CPROVER_object_upto(ctx->ctr, 16),
                  ctx->aes,
                  __CPROVER_object_upto(output, (size_t)size))
{
    u8 stream[16];
    u32 i;
...
__CPROVER_requires(__CPROVER_is_fresh(key, 16 * sizeof(u8)))
__CPROVER_requires(__CPROVER_is_fresh(iv, 16 * sizeof(u8)))
__CPROVER_assigns(__CPROVER_object_upto(ctx->iv, 16), ctx->aes)
{
    aes_setkey_enc(&ctx->aes, key, 128);
    ctr_set_iv(ctx, iv);
}
