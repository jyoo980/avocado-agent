...
#define GET_BITS_BUFSIZE 4096

static uint32_t get_bits(bs_t *bs, int n)
    __CPROVER_requires(__CPROVER_is_fresh(bs, sizeof(*bs)))
    __CPROVER_requires(n >= 1 && n <= 16)
    __CPROVER_requires(bs->pos >= 0 && bs->pos <= 8 * (GET_BITS_BUFSIZE - 8))
    __CPROVER_requires(bs->limit >= 0 && bs->limit <= 8 * (GET_BITS_BUFSIZE - 8))
    __CPROVER_requires(__CPROVER_is_fresh(bs->buf, GET_BITS_BUFSIZE))
...
int read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
    ...
    __CPROVER_requires(bs->limit >= 0 && bs->limit <= 8 * (GET_BITS_BUFSIZE - 8))
    __CPROVER_requires(__CPROVER_is_fresh(bs->buf, GET_BITS_BUFSIZE))
    __CPROVER_requires(__CPROVER_is_fresh(gr, 4 * sizeof(*gr)))
    __CPROVER_requires(__CPROVER_is_fresh(hdr, 4 * sizeof(*hdr)))
