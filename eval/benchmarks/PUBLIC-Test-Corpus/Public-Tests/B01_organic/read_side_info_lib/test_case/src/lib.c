...
static uint32_t get_bits(bs_t *bs, int n)
    ...
    __CPROVER_requires(n >= 0 && n <= 16)
    __CPROVER_requires(bs->pos >= 0 && bs->limit >= 0)
    __CPROVER_requires(bs->limit <= 1024 * 1024)
    __CPROVER_requires(__CPROVER_is_fresh((void *)bs->buf, (bs->limit >> 3) + 4))
...
int read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
    __CPROVER_requires(__CPROVER_is_fresh(bs, sizeof(*bs)))
    __CPROVER_requires(bs->pos >= 0 && bs->limit >= 0 && bs->limit <= 1024 * 1024)
    __CPROVER_requires(__CPROVER_is_fresh((void *)bs->buf, (bs->limit >> 3) + 4))
