...
static uint32_t get_bits(bs_t *bs, int n)
    ...
    __CPROVER_requires(bs->pos >= 0 && bs->pos <= 1000000)
    __CPROVER_requires(bs->limit >= 0 && bs->limit <= 1000000)
    __CPROVER_requires(__CPROVER_is_fresh((void *)bs->buf, 1000000 + 8))
...
int dequantize_granule(float *grbuf, bs_t *bs, L12_scale_info *sci,
    ...
    __CPROVER_requires(bs->limit >= 0 && bs->limit <= 1000000)
    __CPROVER_requires(__CPROVER_is_fresh((void *)bs->buf, 1000000 + 8))
    __CPROVER_requires(__CPROVER_forall { int i; (0 <= i && i < 64) ==> (sci->bitalloc[i] <= 16) })
    __CPROVER_requires(__CPROVER_is_fresh(grbuf, 5000 * sizeof(float)))
