...
__CPROVER_requires(__CPROVER_is_fresh(d, 8))
__CPROVER_assigns(__CPROVER_object_upto(d, 8))
{
    d[0] = (tflac_u8)(n);
    d[1] = (tflac_u8)(n >> 8);
    d[2] = (tflac_u8)(n >> 16);
...
__CPROVER_requires(__CPROVER_is_fresh(samples, sizeof(tflac_s32) * 1024))
