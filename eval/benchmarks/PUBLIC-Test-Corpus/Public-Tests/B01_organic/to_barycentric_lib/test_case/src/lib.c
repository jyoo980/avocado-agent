...
static lm_vec2 lm_v2(float x, float y)
    __CPROVER_requires(x == x && y == y)
    __CPROVER_assigns()
    __CPROVER_ensures(__CPROVER_return_value.x == x && __CPROVER_return_value.y == y)
{
    lm_vec2 v = {x, y};
    return v;
}
...
static lm_vec2 lm_sub2(lm_vec2 a, lm_vec2 b)
    __CPROVER_requires(a.x >= -1.0e18f && a.x <= 1.0e18f && a.y >= -1.0e18f && a.y <= 1.0e18f)
    __CPROVER_requires(b.x >= -1.0e18f && b.x <= 1.0e18f && b.y >= -1.0e18f && b.y <= 1.0e18f)
    __CPROVER_assigns()
    __CPROVER_ensures(__CPROVER_return_value.x == a.x - b.x &&
