...
__CPROVER_requires(__CPROVER_is_fresh(info, sizeof(*info)))
__CPROVER_requires(__CPROVER_is_fresh(data, sizeof(struct caf_header) + sizeof(struct caf_chunk) + sizeof(struct caf_audio_description) + sizeof(struct caf_packet_table) + sizeof(struct caf_data) + sizeof(struct ima_block)))
__CPROVER_assigns(*info)
