...
int ima_parse(struct ima_info *info, const void *data)
    /* The chunk walk advances by the (byte-swapped) chunk size, and dereferences
       desc/pakt after the loop, so a sound proof must hand it a well-formed CAF
       layout. data is pinned to: header | 'desc' chunk (size = sizeof desc) |
       audio_description | 'pakt' chunk (size = sizeof pakt) | packet_table |
       'data' chunk. With concrete chunk sizes the loop runs exactly 3 times and
       every pointer is in bounds; desc/pakt are non-NULL before use. The stored
       big-endian fields are the byte-swaps of the values the code compares
       against. sample_rate is bounded so the double->u64 store is in range. */
    __CPROVER_requires(
        __CPROVER_is_fresh(info, sizeof(struct ima_info)) &&
        __CPROVER_is_fresh(data, 256) &&
        ((const struct caf_header *)data)->type == ima_bswap32(0x63616666u) &&
        ((const struct caf_header *)data)->version == ima_bswap16(1u) &&
        ((const struct caf_chunk *)((const char *)data +
            sizeof(struct caf_header)))->type == ima_bswap32(0x64657363u) &&
        ((const struct caf_chunk *)((const char *)data +
            sizeof(struct caf_header)))->size ==
            (ima_s64_t)ima_bswap64(sizeof(struct caf_audio_description)) &&
        ((const struct caf_audio_description *)((const char *)data +
            sizeof(struct caf_header) + sizeof(struct caf_chunk)))->format_id ==
            ima_bswap32(0x696d6134u) &&
        ((const struct caf_audio_description *)((const char *)data +
            sizeof(struct caf_header) + sizeof(struct caf_chunk)))->sample_rate
            >= 0.0 &&
        ((const struct caf_audio_description *)((const char *)data +
            sizeof(struct caf_header) + sizeof(struct caf_chunk)))->sample_rate
            <= 1000000.0 &&
        ((const struct caf_chunk *)((const char *)data +
            sizeof(struct caf_header) + sizeof(struct caf_chunk) +
            sizeof(struct caf_audio_description)))->type ==
            ima_bswap32(0x70616b74u) &&
        ((const struct caf_chunk *)((const char *)data +
            sizeof(struct caf_header) + sizeof(struct caf_chunk) +
            sizeof(struct caf_audio_description)))->size ==
            (ima_s64_t)ima_bswap64(sizeof(struct caf_packet_table)) &&
        ((const struct caf_chunk *)((const char *)data +
            sizeof(struct caf_header) + 2 * sizeof(struct caf_chunk) +
            sizeof(struct caf_audio_description) +
            sizeof(struct caf_packet_table)))->type == ima_bswap32(0x64617461u))
