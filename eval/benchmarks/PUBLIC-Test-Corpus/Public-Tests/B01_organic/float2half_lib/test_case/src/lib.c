...
static const uint16_t m__base[512] = {
...
static const uint8_t m__shift[512] = {
...
uint16_t float2half(float flt)
    /* j is masked to 9 bits (0..511) so both 512-entry tables are in bounds;
       every shift amount in m__shift is < 32. No memory is modified. */
    __CPROVER_assigns()
{
    union {
        float flt;
